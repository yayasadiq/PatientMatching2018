...
public class PatientGenerator extends Generator {
    private static final int NBR_OF_CHANGES_PER_LINE = 2;
	...
	public void makeData(int nbrLines) throws IOException {
		final int NBR_OF_LINES_TO_GENERATED = 50;
        Random rand = new Random();
        int cursor = 2;
        int curNbrLine = NBR_OF_LINES_TO_GENERATED;
        List<List<Integer>> generatedValues = new ArrayList<>();
        csvWriter.addLines(TITLES);
		for (int i = 0; i < NBR_OF_LINES_TO_GENERATED; i++) {
			List<Integer> line = new ArrayList<>();
			csvWriter.writeCell(i);
			line.add(i);
			int generatedValue = generateAge(rand);
			csvWriter.writeCell(generatedValue);
			line.add(generatedValue);
			while(cursor < 15) {
				generatedValue = generateSeverity(rand);
				csvWriter.writeCell(generatedValue);
				line.add(generatedValue);
				cursor = cursor + 1;
			}
			while(cursor < 27) {
				generatedValue = generateControlLevel(rand);
				csvWriter.writeCell(generatedValue);
				line.add(generatedValue);
				cursor = cursor + 1;
			}
			while(cursor < 164) {
				generatedValue = generateDrug(rand);
				csvWriter.writeCell(generatedValue);
				line.add(generatedValue);
				cursor = cursor + 1;
			}
			generatedValue = generateResult(rand);
			csvWriter.writeCell(generatedValue);
			line.add(generatedValue);
